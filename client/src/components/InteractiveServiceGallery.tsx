  ...
  return <div className={`pp-service-gallery pp-service-gallery--${variant}`}>
    {galleryServices.map((service, index) => {
      ...
      return <article key={service.title} tabIndex={variant === "services" ? -1 : 0} className={`pp-service-card ${active ? "is-active" : ""}`} onMouseEnter={() => setActiveService(index)} onMouseLeave={() => setActiveService(null)} onFocus={() => setActiveService(index)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setActiveService(null); }}>
        <div className="pp-service-card-top"><span>{service.number} · Care path</span><Link href="/request" aria-label={`Start a service request about ${service.title}`}><span className="pp-service-card-request-label">Start a request</span><ArrowUpRight size={17} /></Link></div>
