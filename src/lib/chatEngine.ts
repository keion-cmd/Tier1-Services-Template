 ...
 */
import { faqs, services, staff, howItWorks, businessConfig, emergencyInfo, paymentInfo, trustStats, type Service } from "@/lib/business-content";
...
const PAYMENT_KEYWORDS = ["payment", "pay", "insurance", "insured", "coverage", "covered", "billing", "invoice"];
// TODO (Phase 6): once data/insurance.ts (canonical insurance provider list) exists, extend
// PAYMENT_KEYWORDS matching below to name-check specific carriers and answer with the real
// accepted-plans list instead of the generic paymentInfo.insuranceNote fallback.
...
export function getChatResponse(userMessage: string): ChatResponse {
  ...
  if (includesAny(normalized, HOURS_KEYWORDS) || includesAny(normalized, LOCATION_KEYWORDS)) {
    // TODO (Phase 8): once data/locations.ts (multi-location slug data) exists, check for a
    // named location/borough/neighborhood match here first and answer with that location's
    // specific address/hours instead of always falling back to the single-location businessConfig.
    return {
  ...
  if (includesAny(normalized, PAYMENT_KEYWORDS)) {
    ...
    return {
      text: `We accept ${paymentInfo.methods.join(", ")}. ${paymentInfo.insuranceNote} Questions about your specific plan? Call us at ${businessConfig.phone}.`,
      topic: "pricing",
    };
  }
